...
struct BudgetsView: View {
    ...
    var body: some View {
        ...
        .task {
            let model = BudgetsViewModel(auth: authState)
            vm = model
            await model.load()
        }
        ...
        VStack(spacing: 0) {
            ...
            if vm.selectedTab == 0 {
                HStack(spacing: 6) {
                    ...
                    Text("Cycle from day \(vm.cycleDay)\(vm.rollover ? " · Rollover on" : "")")
    ...
    private func monthlyContent(vm: BudgetsViewModel) -> some View {
        if vm.monthlyCategories.isEmpty {
            ...
            OverallBudgetCard(
                budget: overall,
                pastReadOnly: !vm.rowEditable(overall),
                onTap: {
                    guard vm.rowEditable(overall) else { return }
                ...
                ForEach(vm.monthlyCategories) { l1 in
                    L1BudgetCard(
                        ...
                        onTap: { catId, name, existing in
                            guard vm.rowEditable(existing) else { return }
    ...
    private func tripContent(vm: BudgetsViewModel) -> some View {
        if vm.tripL2s.isEmpty {
            ContentUnavailableView("No trip sub-categories",
                systemImage: "airplane",
                description: Text("Add sub-categories under \"Trip\" in Categories"))
        } else {
            AdaptiveGrid(minColumnWidth: 380, spacing: 8) {
                ForEach(vm.tripL2s) { l2 in
                    TripL2Card(
                        l2: l2,
                        vm: vm,
                        ...
                        onTapL3: { catId, name, existing, parentBudget in
                            editingBudget = BudgetEditTarget(
                                categoryId: catId, categoryName: name, existing: existing, isTrip: false,
...
private struct OverallBudgetCard: View {
    let budget: Budget?
    let pastReadOnly: Bool
    let onTap: () -> Void

    private var notSet: Bool { pastReadOnly && (budget == nil || budget?.cycleExists == false) }
    ...
    var body: some View {
        VStack(spacing: 0) {
            HStack(spacing: 10) {
                ...
                BudgetCellButton(budget: budget, notSet: notSet, disabled: pastReadOnly, onTap: onTap)
            }
        ...
        VStack(spacing: 0) {
            ...
            Button { withAnimation(.easeInOut(duration: 0.2)) { expanded.toggle() } } label: {
                HStack(spacing: 10) {
                    ...
                    if budget == nil && vm.spentRecursive(l1) > 0 {
                        Text(vm.spentRecursive(l1).shortINR).font(.caption2).foregroundStyle(.secondary)
                    }
                    BudgetCellButton(budget: budget, notSet: notSet, disabled: pastReadOnly) { onTap(l1.id, l1.name, budget) }
    ...
    var body: some View {
        VStack(spacing: 0) {
            HStack(spacing: 8) {
                ...
                if budget == nil && spent > 0 { Text(spent.shortINR).font(.caption2).foregroundStyle(.secondary) }
                BudgetCellButton(budget: budget, notSet: notSet, disabled: pastReadOnly, onTap: onTap)
            }
            .padding(.horizontal, 14).padding(.vertical, 10)
            ...
            ForEach(l2.children) { l3 in
                ...
                HStack(spacing: 8) {
                    ...
                    if l3b == nil && l3s > 0 { Text(l3s.shortINR).font(.caption2).foregroundStyle(.secondary) }
                    BudgetCellButton(budget: l3b, notSet: l3NotSet, disabled: l3ReadOnly) { onTapL3(l3) }
                }
                .padding(.horizontal, 14).padding(.vertical, 8)
                if let b3 = l3b {
        ...
        VStack(spacing: 0) {
            Button { withAnimation(.easeInOut(duration: 0.2)) { expanded.toggle() } } label: {
                HStack(spacing: 10) {
                    ...
                    .frame(maxWidth: .infinity, alignment: .leading)
                    BudgetCellButton(budget: budget) { onTapL2(l2.id, l2.name, budget) }
                    Image(systemName: expanded ? "chevron.up" : "chevron.down")
                        .font(.caption).foregroundStyle(.secondary)
                }
                .padding(.horizontal, 14).padding(.vertical, 10)
                ...
                if l2.children.isEmpty {
                    ...
                    ForEach(l2.children) { l3 in
                        ...
                        HStack(spacing: 8) {
                            ...
                            BudgetCellButton(budget: l3b) { onTapL3(l3.id, l3.name, l3b, budget) }
                        }
                        .padding(.horizontal, 14).padding(.vertical, 10)
                        if let b3 = l3b {
                            ProgressBarView(spent: b3.spent, total: b3.effectiveAmount, showLabel: false)
...
struct SetBudgetSheet: View {
    ...
    init(target: BudgetEditTarget, vm: BudgetsViewModel, onDone: @escaping () -> Void) {
        ...
        _amountStr = State(initialValue: seed.map { String(Int($0)) } ?? "")
    ...
    private var exceedsMax: Bool {
        guard let max = target.maxAllowed, let amount = Double(amountStr) else { return false }
        return amount > max
    }
        ...
        NavigationStack {
            ...
            .toolbar {
                ...
                ToolbarItem(placement: .confirmationAction) {
                        ...
                        Task {
                            if target.isMonthMode, let existing = target.existing {
                                ...
                                let req = BudgetRequest(
                                    ...
                                    rolloverEnabled: target.isTrip ? false : vm.rollover
                                )
                    ...
                    .disabled(amountStr.isEmpty || exceedsMax)
                }
            }
        }
    }
...
private struct BudgetSettingsSheet: View {
    ...
    init(vm: BudgetsViewModel) {
        ...
        _rollover = State(initialValue: vm.rollover)
    }
    ...
    var body: some View {
        NavigationStack {
            Form {
                ...
                Section("Rollover") {
                    Toggle("Carry forward unused budget", isOn: $rollover)
                }
            }
            ...
            .toolbar {
                ToolbarItem(placement: .cancellationAction) { Button("Cancel") { dismiss() } }
                ToolbarItem(placement: .confirmationAction) {
                    Button("Save") { vm.cycleDay = cycleDay; vm.rollover = rollover; dismiss() }
                }
            }
        }
    }
...
private struct BudgetCellButton: View {
    ...
    var body: some View {
        if notSet {
            ...
            Button(action: onTap) {
                if let b = budget {
                    Text(b.effectiveAmount.shortINR).font(.caption.bold()).foregroundStyle(Color.accentColor)
                        .padding(.horizontal, 8).padding(.vertical, 4)
                } else {
...
private func budgetProgress(_ b: Budget) -> Double {
    b.effectiveAmount > 0 ? min(b.spent / b.effectiveAmount, 1) : 0
}
